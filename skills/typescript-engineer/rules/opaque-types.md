...
Assertion functions validate and narrow the type by throwing on invalid input. For full details, syntax rules, and examples, see [assertion-functions.md](assertion-functions.md).

```typescript
type ValidEmail = Opaque<string, "ValidEmail">;

// Must use function declaration, not arrow — see assertion-functions.md
...
async function createUser(data: { email: string }): Promise<User> {
  ...
  return await saveUser({ email: data.email });
}
```
