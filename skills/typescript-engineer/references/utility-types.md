...
- [`Awaited<T>`](#awaitedt)
- [Pattern: Wrapping External Library Functions](#pattern-wrapping-external-library-functions)
...
Prevents a type parameter position from contributing to inference. Useful when a generic has multiple use sites and you want one site to drive inference while the other just consumes it.
...
createFSM("off", ["on", "off", "broken"]);
//                              ^^^^^^^^ Error: "broken" is not assignable to "off" | "on"
```
...
getOrDefault<"a" | "b">("a", "c"); // Error: "c" is not "a" | "b"
```

**Event emitters where the event name drives the payload type:**
